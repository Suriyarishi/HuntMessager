  ...
  return (
    <div className="flex flex-col h-full bg-[#F4F7FA] overflow-hidden relative">
      <header className="p-6 pb-2 flex items-center gap-4 sticky top-0 bg-[#F4F7FA]/80 backdrop-blur-xl z-20">
        ...
        <h1 className="text-xl font-black text-[#1F2937]">Linked Devices</h1>
      </header>

      <div className="p-6 flex-1 overflow-y-auto hide-scrollbar pb-24">
        <div className="p-8 bg-[#1F2937] rounded-[40px] mb-10 text-white flex flex-col items-center text-center shadow-2xl relative overflow-hidden border-4 border-white/5">
        ...
        <div className="space-y-4">
          <p className="text-[10px] font-black text-[#6B7280] uppercase tracking-[0.2em] px-4 mb-2">Your Devices</p>
          ...
          {devices.map(device => (
            ...
            >
              <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 ${device.isCurrent ? 'bg-gradient-to-br from-[#2FED9A] to-[#12C784] text-white shadow-lg' : 'neumorphic-inset text-[#94A3B8]'}`}>
                {getDeviceIcon(device.type)}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <h3 className="font-extrabold text-sm text-[#1F2937] truncate">{device.name}</h3>
                  {device.isCurrent && (
                ...
                <p className="text-[10px] text-[#6B7280] font-black uppercase tracking-tight opacity-70">{device.location}</p>
                <p className={`text-[10px] font-black uppercase mt-1.5 tracking-widest ${device.lastActive === 'Active Now' ? 'text-[#12C784]' : 'text-[#94A3B8]'}`}>
                  {device.lastActive}
                </p>
              </div>
              {!device.isCurrent && (
        ...
        <p className="mt-12 text-[10px] text-[#6B7280] text-center px-8 leading-relaxed font-black uppercase tracking-[0.1em] opacity-60">
          Logged in devices can access your history. Logout from any device you don't recognize.
        </p>
      </div>

      {isLinking && (
        <div className="absolute inset-0 bg-[#F4F7FA]/98 backdrop-blur-xl z-50 flex flex-col items-center justify-center animate-[fadeIn_0.3s_ease-out]">
          <div className="relative w-72 h-72 mb-12 neumorphic-elevated p-6 flex items-center justify-center">
            ...
            <div className="w-full h-full bg-[#EAEEF3] opacity-50 flex items-center justify-center rounded-[24px]">
              <svg className="w-32 h-32 text-[#1F2937]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z" />
              </svg>
            </div>
          </div>

          <h2 className="text-2xl font-black mb-3 text-[#1F2937]">Scan QR Code</h2>
          <p className="text-sm text-[#6B7280] text-center px-12 mb-12 font-bold uppercase tracking-wider opacity-80">
            Visit <span className="text-[#12C784] underline">hunt.chat</span> on your computer
          </p>

          <button
            onClick={() => setIsLinking(false)}
            className="px-10 py-4 bg-[#1F2937] text-white font-black rounded-2xl shadow-xl active:scale-95 transition-all text-sm uppercase tracking-widest"
          >
            Cancel
          </button>
        </div>
